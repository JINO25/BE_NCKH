...
exports.findUser = async (email) => {
  const q = query(
    collection(firebaseStore.db, "user"),
    where("email", "==", email)
  );
...
// hàm thứ 2
exports.addDataWaterVolume = async (
  humd,
  waterVolume05,
  waterVolume085,
  waterVolume06,
...
function roundToHour(date) {
  ...
  if (minutes < 50) {
    d.setMinutes(0, 0, 0); // HH:00:00
  } else {
    d.setHours(d.getHours() + 1);
    d.setMinutes(0, 0, 0); // (HH+1):00:00
  }
...
exports.addDataForWeatherToday = async (
  maxTemp,
  minTemp,
  temp,
  icon,
...
//   const now = new Date();
//   const rounded = roundToHour(now); // đã làm tròn giờ
...
//     solar,
//     titleOfWeather,
//     currentTime: rounded.getTime(),
//     timestamp: rounded.toLocaleDateString(),
//   });
};
...
exports.getWeatherToday = async () => {
  ...
  const q = query(
    collection(firebaseStore.db, "weatherToday"),
...
exports.getWeather7days = async () => {
  ...
  const q = query(
    collection(firebaseStore.db, "weather7days"),
    where("timestamp", "==", today.toLocaleDateString().toString()),
    orderBy("currentTime", "desc"),
    limit(1)
  );
...
exports.getAllGardens = async (user) => {
  ...
  let data = [];
  const q = query(
    collection(firebaseStore.db, "garden"),
...
exports.getCropTypes = async (user) => {
  const userID = await this.findUser(user.email);
  const qBase = query(
    collection(firebaseStore.db, "cropType"),
    where("user", "==", userID.id),
    orderBy("timestamp", "desc")
  );
